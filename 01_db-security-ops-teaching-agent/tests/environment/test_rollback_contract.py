...
def test_rollback_has_one_documented_entry_point_and_one_executable_source() -> None:
    ...
    assert helper.count("$Marker = '^Close executable rollback review gaps$'") == 1
...
def create_isolated_rollback_repository(tmp_path: Path) -> dict:
    ...
    (agent / ".env").write_text("TEST_ONLY=not-a-secret\n", encoding="utf-8")
    ...
    git(work, "commit", "-m", "Close executable rollback review gaps")
...
def test_ast_gate_ignores_comments_and_unused_documentation_strings(tmp_path: Path) -> None:
    ...
    harmless.write_text(
        "# Invoke-Native -File 'git' -Arguments @('reset','--hard')\n"
        "$documentation = \"git clean -fd is prohibited\"\n"
        "Write-Output 'safe'\n",
        encoding="utf-8",
    )
    assert dangerous_ast_commands(powershell_command_asts(harmless)) == []
...
def rollback_source_candidates(root: Path) -> list[Path]:
    candidates = []
    for path in root.rglob("*"):
        ...
        tokens = set(executable_tokens(text))
        if (
            path.name.lower() == "rollback-environment.ps1"
            or any(token.endswith("rollback-environment.ps1") for token in tokens)
            or "invoke-rollback" in tokens
            or "25351d020a9ef413d9288010028acba579fe7938" in tokens
            or {"git", "revert", "--no-commit"} <= tokens
        ):
...
def test_source_enumeration_detects_duplicate_supported_entry_points(
    ...
    canonical.write_text("$Marker = '^Close executable rollback review gaps$'", encoding="utf-8")
    duplicate.write_text("powershell ./rollback-environment.ps1", encoding="utf-8")
    assert rollback_source_candidates(tmp_path) == [duplicate, canonical]
...
def test_repository_has_exactly_one_supported_executable_rollback_source() -> None:
    assert rollback_source_candidates(ROOT) == [ROLLBACK]
...
def test_log_guards_are_executable_before_staging() -> None:
    ...
    assert append_guards[0] < secret_guards[0] < stage_logs[0]
